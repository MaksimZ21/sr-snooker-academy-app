...
export async function POST(req: Request) {
  try {
    ...
    const [{ data: coachRow }, { data: studentRow }] = await Promise.all([
      ...
      // Students are deliberately not active-filtered — a student who
      // stopped coming, or an is_tournament_only customer (always
      // active: false by design), must still be able to log in.
      db.from("students").select("email").or(`phone.eq.${local},phone.eq.${intl}`).maybeSingle(),
    ]);
